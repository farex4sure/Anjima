...
$getLatest = mysqli_query($conn, "SELECT * from chat WHERE mto='$otherPhone' OR mfrom='$otherPhone' ORDER BY id DESC LIMIT 1");
...
$getcs = mysqli_query($conn, "SELECT * FROM vendors WHERE phone='$otherPhone'");
while ($cs = mysqli_fetch_assoc($getcs)) {
$cid = $cs['phone'];

// get users fullname using his id
$getcname = mysqli_query($conn, "SELECT * FROM vendors WHERE phone='$cid'");
...
$list.='<a href="interaction.php?interactwith=' . base64_encode($cid) . '" class="flex items-center w-full px-3 py-2 text-sm transition duration-150 ease-in-out border-b border-gray-300 cursor-pointer hover:bg-gray-100 focus:outline-none">
    <img class="object-cover w-10 h-10 rounded-full" src="../vendors/vendor/'.$pic.'" alt="username" />
